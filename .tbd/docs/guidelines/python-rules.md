...
- Important: BE SURE you read and understand the project setup by reading the
  pyproject.toml file and (if present) the Makefile.

- If uv is available, use uv for running all code and managing dependencies, not `pip`
  or `python` commands.
...
- Always use full, absolute imports for paths.
  ...
  refactor. Use `from toplevel_pkg.module1.module2 import ...` instead.
...
- NEVER write `assert False`. If a test reaches an unexpected branch and must fail
  ...
  This is typical best practice in Python since assertions can be removed with
  optimization.
...
- Do NOT list args and return values if they’re obvious.
  In the above examples, you do not need an `Arguments:` or `Returns:` section, since it
