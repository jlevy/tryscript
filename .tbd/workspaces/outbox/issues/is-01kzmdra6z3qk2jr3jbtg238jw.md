...
---
A project config that default-exports null, an array, or a primitive receives a
non-fatal validation warning but is still consumed as TryscriptConfig; property reads
then crash.
Preserve the warning, normalize unusable top-level values to an empty config,
and run requested files safely.
