...
---
fast-glob documents that matches are returned in arbitrary order, but run executes them
directly. Report ordering and --fail-fast selection can vary by filesystem and
concurrency.
Sort absolute paths with a deterministic ordinal comparator before execution
and cover it.
